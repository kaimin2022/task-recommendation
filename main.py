...
def evaluation(model, helper, testRatings, testNegatives, K, type_m):
    model.eval()
    (compe, hits, ndcgs) = helper.evaluate_model(model, testRatings, testNegatives, K, type_m)
    a = []
...
if __name__ == '__main__':
    ...
    for epoch in range(config.epoch):
        ...
        # 评估模型
        t2 = time()
        c, u_hr, u_ndcg = evaluation(agree, helper, dataset.user_testRatings, dataset.user_testNegatives, config.topK, 'user')
        print('User Iteration %d [%.1f s]: HR = %.4f, NDCG = %.4f, [%.1f s]' % (
            epoch, time() - t1, u_hr, u_ndcg, time() - t2))
        #print('个人满意度为：%.4f' % (score))
        print('任务完成率为：%.4f' % (c))
        c, hr, ndcg = evaluation(agree, helper, dataset.group_testRatings, dataset.group_testNegatives, config.topK, 'group')
        print(
