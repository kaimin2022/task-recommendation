...
# 这里的评估是针对测试集，然后测试集里面有一个对应的用户-物品，另外负样本里面还有这个用户的100个未交互过的物品，所以会预测101个物品的分数
...
class Helper(object):
    ...
    def evaluate_model(self, model, testRatings, testNegatives, K, type_m):
        ...
        for idx in range(len(testRatings)): #对每一个样本进行评估
            ...
            if hr == 1 and testRatings[idx][1] not in compe:
                compe.append(testRatings[idx][1])
            hits.append(hr)
            ndcgs.append(ndcg)
        return (compe,hits, ndcgs)
    ...
    def eval_one_rating(self, model, testRatings, testNegatives, K, type_m, idx):
        ...
        # 满意度
        #u_score = predictions.data.numpy()[50]
